...
`CONFIG_KSU=y`、KPM，以及经过 Xiaomi 5.4 适配的 SUSFS 1.5.5。
...
SUSFS 固定使用
[simonpunk/susfs4ksu](https://gitlab.com/simonpunk/susfs4ksu/-/tree/kernel-5.4)
的 `kernel-5.4` 提交 `76affd70abf61d77feb0a132f61365d6848505df`。工作流复制
官方 `susfs.c` 与头文件，应用官方 5.4 内核补丁，再对 Xiaomi 的 mount KABI、
`fdinfo` 和 `task_mmu` 差异应用小范围适配。首版只打开 `SUS_PATH`、`SUS_MOUNT`
和 `SUS_KSTAT`；`SUS_SU`、bootconfig/cmdline 伪装、uname 伪装、符号隐藏、
open redirect 和 SUS_MAP 保持关闭。
